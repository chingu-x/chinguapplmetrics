...
import createPaidMemberByDayMetrics from '../util/createPaidMemberByDayMetrics'

export default function PaidMembersByDay(props) {
  ...
  useEffect(() => {
    async function fetchData() {
      if (fileContents !== '') {
        setPaidMemberJSON(await createPaidMemberByDayMetrics(fileContents))
  ...
  return (
    <React.Fragment>
      <Typography variant="h6" color="inherit" noWrap>
        Paid Member Signups by Day of Month
      </Typography>
      { isDataLoaded
